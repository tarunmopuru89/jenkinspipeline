...
node {
    ...
    // -------------------------------------------------------------------------
             ...
             stage('Authorize DevHub') {
                rc = command  "\"${toolbelt}\" sfdx force:auth:jwt:grant --instanceurl ${SF_INSTANCE_URL} --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile ${SERVERKEY} --setdefaultdevhubusername --setalias HubOrg"
                if (rc != 0) {
